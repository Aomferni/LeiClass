...
void
testAbs1(void){
    ensure(floatEqual(abs1(-3), 3), "product test error 1");
    ensure(floatEqual(abs1(5), 5), "product test error 1");
}
...
testMin1(void){
    ...
    float s1 = min1(v1);
    // 判断浮点数相等，建议使用 floatEqual 函数
    ensure(floatEqual(s1, 1), "min1 test error 1");
    //
    vector<float> v2 = {1, 2, -3, 4, 5};
    float s2 = min1(v2);
    ensure(floatEqual(s2, -3), "min1 test error 2");
}
...
void
testSum1(void){
    ensure(sum1(4) == -2, "sum test errror 1");
    ensure(sum1(3) == 2, "sum test error 2");
    ensure(sum1(5) == 3, "sum test error 3");
}
...
void
testSum2(void){
    ensure(sum2(4) == 4, "sum2 test errror 1");
    ensure(sum2(3) == 0, "sum2 test error 2");
    ensure(sum2(5) == -1, "sum2 test error 3");
}
// 作业 7
//
// 实现 fac 函数
